public class Booking
{
    ...
    public Booking(Guid roomId, DateTime startTime, DateTime endTime)
    {
        ...
        startTime = RoundStartDown(startTime);
        endTime = RoundEndUp(endTime);
    ...
    private static DateTime RoundStartDown(DateTime time)
    {
        var roundedMinutes = time.Minute < 30 ? 0 : 30;
        return new DateTime(time.Year, time.Month, time.Day, time.Hour, roundedMinutes, 0);
    }

    private static DateTime RoundEndUp(DateTime time)
    {
        if (time.Minute == 0 || time.Minute == 30)
            return time; // вже на межі блоку
